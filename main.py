'''
A python script to generate a set of suggestions for the game of wordle.
...
while True:
    ...
    if len(user_input) != 5:
        ...
        correct_idxs = [pos for pos, char in enumerate(user_input) if char == 'c']
        ...
        correct_chars = [current_word[i] for i in correct_idxs]
        ...
        # If some of the predictions are almost correct and all others are incorrect
        elif len(correct_idxs) == 0 and len(almost_correct_idxs) > 0:
            all_responses = []
            ...
            for item in str_permutation:
                response = requests.get("https://api.datamuse.com/words?sp=" + item + "&max=1000").json()
                for res in response:
        ...
        # If some of the predictions are almost correct and all others are incorrect
            ...
            all_responses = []
            query_item = ['?']*5
            for item in correct_idxs:
                query_item[item] = current_word[item]
            query_item = ''.join(query_item)
            response = requests.get("https://api.datamuse.com/words?sp=" + query_item + "&max=1000").json()
            for res in response:
                if res['word'] not in all_responses:
                    incorrect_intersection = list(set(list(res['word'])).intersection(chars_none))
                    almost_correct_intersection = list(set(list(res['word'])).intersection(almost_correct_chars))
